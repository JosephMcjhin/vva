...
【响应门控】
只有当用户明确主动要求描述当前画面、环境或前方目标时，才允许输出语音或文字。
除上述主动描述请求外，任何语句都不要回复，包括闲聊、确认、找东西、问人物、读文字和问这是什么。
导航请求由专用导航模块处理，必须保持绝对沉默，不要输出任何语音或文字。
...
- 若用户问文本 → 读取最重要的文字内容。
- **【核心禁令】若用户请求“导航”相关功能（如“开始导航”、“停止导航”、“去某个地方”等），你必须保持绝对沉默，不要回复任何语音或文字，也不要进行确认，因为系统会由专用导航模块处理。**
...
所有输出均需口语化、易理解，并适合盲人听觉体验。
    ...
    fun connect(): Boolean {
        ...
        val currentConversation = OmniRealtimeConversation(param, object : OmniRealtimeCallback() {
        ...
        val connectThread = Thread({
            try {
                // The SDK waits on a latch and does not release it from onFailure().
        ...
        if (connectThread.isAlive) {
            connectThread.interrupt()
            try {
                currentConversation.close(1000, "connect timeout")
            } catch (_: Exception) {
                // The SDK may not have completed its socket setup yet.
            }
            isConnecting = false
            onError("连接超时（${CONNECT_TIMEOUT_MS / 1000}秒），请检查手机网络、VPN或私有DNS")
            return false
        }

        connectError?.let { error ->
            isConnecting = false
            onError("连接失败：${error.message ?: error.javaClass.simpleName}")
            return false
        }

        return try {
            currentConversation.updateSession(
            ...
            isConnecting = false
            onError("会话配置失败：${error.message ?: error.javaClass.simpleName}")
            false
        }
    }
